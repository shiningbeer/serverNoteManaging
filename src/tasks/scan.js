      ...
      nodeApi.zmapTask.syncProgress(url, token, _id.toString(), 120000, async (code, body) => {
        if (code == 200) {
          ...
          if (complete) {
            ...
            logger.warn('【zmaptask】-【Complete!】of node【%s】： nodetask(%s) of task(%s)!', name, _id, taskId)
...
const getResults = async () => {
  ...
  for (var task of tasks) {
    ...
    if (nodetasks.length == 0) {
      logger.info('【zmaptask】-【result】: Complete collecting results for task %s', task.name)
    ...
    for (var nodetask of nodetasks) {
      ...
      nodeApi.results.get(url, token, nodetaskid, resultReceived, 10, (code, body) => {
        if (code == 200) {
          logger.debug(body)
          logger.info('【zmaptask】-【result】: Receive frome node 【%s】for nodetask【%s】of task【%s】', name, nodetaskid, taskName)
