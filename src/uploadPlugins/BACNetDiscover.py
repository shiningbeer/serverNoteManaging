...
##########
        ...
        if data.startswith('\x81',0,3): #响应以0x81起始，说明是BACNet,写入相关文件中
            try:
                dictOfBACNetInfo = GetInfo(TargetAddr, s, data)
            except Exception,e:
    ...
    current_time = time.strftime('%Y-%m-%d_%X', time.localtime())
    all_dict["IP"] = IP
...
def vendor_lookup(vennum):
    if vennum >= 0 and vennum <= 728:
        vendorname = vendor_id[str(vennum)]
    else:
        vendorname = "Unknown Vendor Number"
    return vendorname


def vendornum_query(socket, TargetAddr):
    vendor_query = dataSwitch('810a001101040005010c0c023FFFFF1978')
    socket.sendto(vendor_query, TargetAddr)
    data,ADDR = socket.recvfrom(BUFSIZE)
    
    if data.startswith('\x81',0): #响应以0x81起始，说明是BACNet
        pass

    value = 0
    if b2a_hex(data[6]) != '50':    
        value = b2a_hex(data[17])   #取第18个字符，这个值决定vendor号是1个或2个字节
    if value == '21':
        int(b2a_hex(data[18:19]),16)
        return vendor_lookup(ord(data[18]))
    elif value == '22':
        int(b2a_hex(data[18:19]),16)
        ord(data[18])     #取字符的整数值
        
        (vendornum,) = struct.unpack('>H', data[18:20])
        return vendor_lookup(vendornum)
    else:
        print 'Error receiving Vendor ID: Invalid BACNet packet'
        return 0
...
def standard_query(socket, query_type, TargetAddr):
    vendor_query = dataSwitch("810a001101040005010c0c023FFFFF1979")
    firmware_query = dataSwitch("810a001101040005010c0c023FFFFF192c")
    appsoft_query = dataSwitch("810a001101040005010c0c023FFFFF190c")
    object_query = dataSwitch("810a001101040005010c0c023FFFFF194d")
    model_query = dataSwitch("810a001101040005010c0c023FFFFF1946")
    desc_query = dataSwitch("810a001101040005010c0c023FFFFF191c")
    location_query = dataSwitch("810a001101040005010c0c023FFFFF193A")
    query = ""
    
    if query_type == "firmware":
        ...
        query = vendor_query
    try:
        socket.sendto(query, TargetAddr)
        data,ADDR = socket.recvfrom(BUFSIZE)
        if data.startswith('\x81',0): #响应以0x81起始，说明是BACNet
            pass
            
        if b2a_hex(data[6]) != '50':    
            return field_size(data)
        else:   #vendor query resulted in an error
            print 'Error receiving Vendor ID: BACNet Error'
    except Exception,e:
        return str(e)


def GetInfo(TargetAddr, s, data):
    to_return={}
    time.sleep(3)
   
    if b2a_hex(data[6]) == '50':
        to_return['error'] = 'BACNet ADPU Type: Error (5) ' + str(b2a_hex(data))
        return to_return
    try:
        to_return["VendorID"] = vendornum_query(s, TargetAddr)
    except Exception,e:
        to_return['VendorID'] = str(e)
    
    to_return["VendorName"] = standard_query(s, "vendor", TargetAddr)
    to_return["Object-identifier"] = int(b2a_hex(data[19:22]),16)
    to_return["Firmware"] = standard_query(s, "firmware", TargetAddr)
    # Application Software Version
    to_return["ApplicationSoftware"] = standard_query(s, "application", TargetAddr)
    # Object Name
    to_return["ObjectName"] = standard_query(s, "object", TargetAddr)
    # Model Name
    to_return["ModelName"] = standard_query(s, "model", TargetAddr)
    # Description       
    to_return["Description"] = standard_query(s, "description", TargetAddr)
    # Location
    to_return["Location"] = standard_query(s, "location", TargetAddr)
