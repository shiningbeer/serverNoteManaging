...
def vendor_lookup(vennum):
	print vennum
	if vendor_id.has_key(str(vennum)):
...
def device_type_lookup(devtype):
	
	if device_type.has_key(str(devtype)):
		DeviceType = device_type[str(devtype)]
	else:
		DeviceType = "Unknown Vendor Number"
	print DeviceType
	return DeviceType

def get_string(data):	#解析数据，删除\x00之后的数据
	string = ""
	for c in data:
		if c == '\x00':
			break
...
def scan(ip,port = 44818):
	...
	try:
		...
		print len(response)
		command = b2a_hex(response[0])
		typeid  = b2a_hex(response[26])
		if command == '63':
			if typeid == '0c':
				#-- vendor number
				(vennum,) = struct.unpack('<H', response[48:50])
				#print int(b2a_hex(response[48:50]))
				#-- look up vendor number
				to_return["Vendor"] = vendor_lookup(vennum) + " (" + str(vennum ) + ")"
				
				product_name_length = ord(response[62])
				
				#-- unpack product name into output table
				to_return["ProductName"] = get_string(response[63:64 + product_name_length])
				print to_return["ProductName"]
				
				#-- unpack the serial number
				(serial ,) = struct.unpack('<I', response[58:62])
				#-- print it out in hex format
				to_return['SerialNumber'] = hex(serial)
				print to_return['SerialNumber']
				
				#-- device type number
				(devnum,) = struct.unpack('<H', response[50:52])
				to_return["DeviceType"] = device_type_lookup(devnum) + " (" + str(devnum) + ")"
				print to_return["DeviceType"]
				
				#	-- unpack product code as a two byte int
				(char1,) = struct.unpack("H", response[52:54])
				to_return["ProductCode"] = char1
				print type(char1)
				
				#-- Revision Number
				char1, char2 = struct.unpack("BB", response[54:56])
				to_return["Revision"] = str(char1) + "." + str(char2)
				print to_return["Revision"]
				
				#-- Device IP, this could be the same, as the IP scanning, or may be actual IP behind NAT
				(dword,) = struct.unpack("!I", response[36:40])		#!代表网络字节序
				to_return["DeviceIP"] = socket.inet_ntoa(struct.pack('!L', dword))
				print to_return["DeviceIP"]
		
	except Exception,e:
		print 'enip-enumerate get error', str(e)
